...
class Abstract:
    ...
    def set_inventory(self, items_data: bytes, inventory: Dict[str, List], length: int, slot: str, lang: str) -> None:
        ...
        for i in range(0, len(items_data), length):
            ...
            print(f"item number:{i // length}, index:{i}, length:{length}, end:{i + length}")
            print(f"slot: {slot}")
    ...
    def set_meseta(self, meseta_data: bytes, inventory: Dict[str, List], slot: str, lang: str) -> None:
        ...
        inventory[lang].append([
            "09" + str(meseta).zfill(7),  # Add prefix to make meseta maximum item code
            item,
            slot
        ])
    ...
    def is_blank(self, item_data: bytes) -> bool:
        ...
        return (sum(item_data[:20]) == 0 or 
                CommonUtil.binary_array_to_hex(item_data) == "000000000000000000000000FFFFFFFF0000000000000000" or
                CommonUtil.binary_array_to_hex(item_data).find("00FF00000000000000000000FFFFFFFF") != -1) 